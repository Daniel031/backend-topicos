...
<div class="denuncias">
  <a class="btn btn-warning" href="{{route('tipos_denuncias.create')}}">Crear</a>
</div>
<table id="myTable" class="table table-striped table-bordered" style="width:100%">
  ...
  <tbody>
    ...
    @foreach($tipos as $user)
      <tr>
        <td >{{$user->nombre}}</td>
        <td >{{$user->descripcion}}</td>
        <td >{{$user->estado?"Habilitado":"Deshabilitado"}}</td>
        <td >
          <form action="{{route('tipos_denuncias.destroy',$user)}}" method="POST">
            @method('DELETE')
            @csrf
            <a href="{{route('tipos_denuncias.edit',$user)}}" class="btn btn-primary"><i class="fa fa-edit">&nbsp;</i></a>
            <a href="{{route('tipos_denuncias.show',$user)}}" class="btn btn-info"><i class="fa fa-eye">&nbsp;</i></a>
